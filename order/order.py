...
def update_sale_order(pid, data_pool, error_ids, write_ids, partner_ids, term_ids):
...
def sync_sale_orders():
    ...
    domain = [('name', 'in', orders.keys())]
    res = sock.execute(DB, UID, PSW, 'sale.order', 'search_read', domain, ['name'])
    write_ids = {rec['name']: rec['id']  for rec in res}

    payment_terms = sock.execute(DB, UID, PSW, 'account.payment.term', 'search_read', [('order_type','=','sale')], ['id','code'])
    ...
    for i in range(WORKERS):
        pid = "Worker-%d" % (i + 1)
        worker = mp.Process(name=pid, target=update_sale_order, args=(pid, data_pool, error_ids, write_ids, partner_ids, term_ids))
